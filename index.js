...
const EMAIL = 'destek.m8@gmail.com';
const PASSWORD = '11551155aA.';
...
async function loginAndGetCookies(email, password, browserPath) {
...
async function runBot() {
  ...
  try {
    const cookies = await loginAndGetCookies(EMAIL, PASSWORD, BROWSER_PATH);
    fs.writeFileSync(COOKIES_PATH, JSON.stringify(cookies, null, 2), 'utf-8');
    fs.writeFileSync(BACKUP_COOKIES_PATH, JSON.stringify(cookies, null, 2), 'utf-8');
    console.log('🔐 Login başarılı, cookie dosyaları oluşturuldu.');

    const browser = await chromium.launch({ headless: false, executablePath: BROWSER_PATH });
    const context = await browser.newContext();
    ...
    for (let i = 1; i <= PAGE_LIMIT; i++) {
      ...
      if (status === 403 || status === 401) {
        ...
        console.log(`🔁 Cookie geçersiz. 10 dk sonra yeniden deneniyor...`);
